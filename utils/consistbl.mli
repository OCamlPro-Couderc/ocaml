...
val check: t -> string -> string option -> Digest.t -> string -> unit
...
val check_noadd: t -> string -> string option -> Digest.t -> string -> unit
      (* Same as [check], but raise [Not_available] if no CRC was previously
           associated with [name]. *)

val set: t -> string -> string option -> Digest.t -> string -> unit
...
val source: t -> string -> string option -> string
      (* [source tbl name] returns the file name associated with [name]
         if the latter has an associated CRC in [tbl].
         Raise [Not_found] otherwise. *)

val extract: (string * string option) list -> t -> (string * string option * Digest.t option) list
...
val filter: ((string * string option) -> bool) -> t -> unit
