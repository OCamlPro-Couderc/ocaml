...
module type S = sig
  ...
  class virtual selector_generic : object
    ...
    method select_operation :
      ...
      Cmm.expression list ->
      Debuginfo.t ->
      Mach_type.Make(Arch).operation * Cmm.expression list
    (* Can be overridden to deal with special arithmetic instructions *)
    method select_condition : Cmm.expression -> Mach_type.Make(Arch).test * Cmm.expression
    (* Can be overridden to deal with special test instructions *)
    method select_store :
      bool -> Arch.addressing_mode -> Cmm.expression ->
      Mach_type.Make(Arch).operation * Cmm.expression
    ...
    (* Return an array of fresh registers of the given type.
       ...
       integer registers. *)
    method insert_op :
      environment -> Mach_type.Make(Arch).operation -> Reg.t array -> Reg.t array -> Reg.t array
    (* Can be overridden to deal with 2-address instructions
       or instructions with hardwired input/output registers *)
    method insert_op_debug :
      environment -> Mach_type.Make(Arch).operation -> Debuginfo.t -> Reg.t array
    ...
    method mark_instr : Mach_type.Make(Arch).instruction_desc -> unit
    ...
    method emit_fundecl : Cmm.fundecl -> Mach_type.Make(Arch).fundecl
    ...
    method extract : Mach_type.Make(Arch).instruction
    method extract_core : end_instr:Mach_type.Make(Arch).instruction -> Mach_type.Make(Arch).instruction
    method insert :
      environment -> Mach_type.Make(Arch).instruction_desc -> Reg.t array -> Reg.t array -> unit
    method insert_debug :
      environment -> Mach_type.Make(Arch).instruction_desc -> Debuginfo.t ->
    ...
    method select_allocation : int -> Mach_type.Make(Arch).operation
    method select_allocation_args : environment -> Reg.t array
    method select_checkbound : unit -> Mach_type.Make(Arch).integer_operation
    ...
    method about_to_emit_call
      : environment
        -> Mach_type.Make(Arch).instruction_desc
        -> Reg.t array
        -> Debuginfo.t
        -> Reg.t array option
    ...
    method insert_prologue
      : Cmm.fundecl
        ...
        -> env:environment
        -> Mach_type.Make(Arch).spacetime_shape option

    val mutable instr_seq : Mach_type.Make(Arch).instruction
