...
let insert_profile rw_exp ex =
  let st = ex.pexp_info.loc_start.Lexing.pos_cnum
  and en = ex.pexp_info.loc_end.Lexing.pos_cnum
  and gh = ex.pexp_info.loc_ghost
  in
  if gh || st = en then
    rw_exp true ex
  else begin
...
and rw_exp iflag sexp =
  ...
  | Pexp_match(sarg, caselist) ->
    ...
    if !instr_match && not sexp.pexp_info.loc_ghost then
  ...
  | Pexp_try(sbody, caselist) ->
    rewrite_exp iflag sbody;
    if !instr_try && not sexp.pexp_info.loc_ghost then
  ...
  | Pexp_ifthenelse(scond, sifso, None) ->
      ...
      rewrite_ifbody iflag sexp.pexp_info.loc_ghost sifso
  | Pexp_ifthenelse(scond, sifso, Some sifnot) ->
      rewrite_exp iflag scond;
      rewrite_ifbody iflag sexp.pexp_info.loc_ghost sifso;
      rewrite_ifbody iflag sexp.pexp_info.loc_ghost sifnot
  ...
  | Pexp_while(scond, sbody) ->
    rewrite_exp iflag scond;
    if !instr_loops && not sexp.pexp_info.loc_ghost
