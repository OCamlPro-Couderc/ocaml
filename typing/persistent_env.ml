...
let is_package_parameter current_unit modname =
  List.exists
    (function CU.Prefix.Pack (_, args) ->
       List.exists (CU.Name.equal modname) args)
...
let check_parameter modname flags functor_unit =
  ...
  is_package_parameter current_unit modname
  ...
  List.iter
    (function
      ...
      | Parameter_of functor_unit ->
          if not (check_parameter ps.ps_name ps.ps_flags functor_unit) then
...
let report_error ppf =
  ...
  | Illegal_import_of_parameter(modname, filename) -> fprintf ppf
      "@[<hov>The file %a@ contains the an interface of a parameter.@ \
       %s is not declared as a parameter for the current unit (-parameter %s) nor@ \
