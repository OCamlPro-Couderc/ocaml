...
and transl_recmodule_modtypes ~persistent env sdecls =
  ...
  let ids =
    List.map (fun x ->
        if persistent then Option.map (Ident.create_persistent) x.pmd_name.txt
        else Option.map (Ident.create_scoped ~scope) x.pmd_name.txt) sdecls
  in
  let approx_env =
    List.fold_left
      (fun env ->
  ...
  let rec check_incl first_time n env s =
    ...
    if n > 0 then begin
          ...
          (fun env (ids, mty_impl) ->
             ...
             | Some (id, id') ->
               let mty_impl' =
                 if first_time
                 then mty_impl
                 else subst_and_strengthen env scope s (Some id) mty_impl
               in
      ...
      let check_inclusion (id, intf, mty_impl, loc) =
        let intf' = Subst.modtype (Rescope scope) s intf
        and mty_impl' = subst_and_strengthen env scope s id mty_impl in
        try
...
let report_error ppf = function
  ...
  | Cannot_implement_recursive_interface modname ->
      fprintf ppf
        "Interface %s is declared as recursive,@ \
