...
let transl_extension_constructor env path ext =
  ...
  in
  let ty =
    match ext.ext_type.ext_ret_type with
      None ->
        Val (Ctype.newconstr
               ext.ext_type.ext_type_path ext.ext_type.ext_type_params)
    | Some ty -> Val ty
  in
  ...
  | Text_rebind(path, lid) ->
      { (transl_path ~loc:ext.ext_loc env path) with
        lb_tt_type = Some ty;
        lb_from = Some "transl_exception_constructor" }

let transl_type_extension env rootpath tyext body =
  let mk l =
    { lb_expr = l;
      lb_env = Some env;
      lb_tt_type = body.lb_tt_type;
      lb_from = Some "transl_type_extension" } in
  List.fold_right
    (fun ext body ->
      let lam =
...
let rec apply_coercion env strict restr arg =
      ...
      name_lambda strict arg (fun id ->
          ...
          let lam =
            ...
            Lprim(Pmakeblock(0, Immutable),
                  List.map (apply_coercion_field env get_field) pos_cc_list)
          in
          wrap_id_pos_list env id_pos_list get_field lam)
  | Tcoerce_functor(cc_arg, cc_res) ->
      let param = Ident.create "funarg" in
      name_lambda strict arg (fun id ->
          mk @@
        Lfunction(Curried, [param],
          apply_coercion env Strict cc_res
            (mk @@ Lapply(mk @@ Lvar id,
                          [apply_coercion env Alias cc_arg (mk @@ Lvar param)],
                          Location.none))))
  | Tcoerce_primitive p ->
      transl_primitive Location.none None env p
  | Tcoerce_alias (path, cc) ->
      name_lambda strict arg
        (fun id -> apply_coercion env Alias cc (transl_normal_path env path))

and apply_coercion_field env get_field (pos, cc) =
  apply_coercion env Alias cc (get_field pos)

and wrap_id_pos_list env id_pos_list get_field lam =
  let mk l = mk_lambda ~from:"apply_wrap_id_pos_list" l in
  ...
  let (lam,s) =
    List.fold_left (fun (lam,s) (id',pos,c) ->
      if IdentSet.mem id' fv then
        ...
        (mk @@ Llet(Alias,id'',
                    apply_coercion env Alias c (get_field pos),lam),
         Ident.add id' (mk @@ Lvar id'') s)
      else (lam,s))
...
let init_shape modl =
  ...
  try
    Some(undefined_location modl.mod_loc,
         mk_lambda ~ty:(Mod modl.mod_type) ~env:modl.mod_env ~from:"init_shape" @@
...
let eval_rec_bindings bindings cont =
    ...
    | (id, Some(loc, shape), rhs) :: rem ->
        ...
        { rem with
          ...
          lb_expr = Llet(Strict, id,
                         mk_lambda ?ty:None ?env:rem.lb_env
                           ~from:"eval_rec_bindings.bind_inits" @@
  ...
  and bind_strict = function
    ...
    | (id, None, rhs) :: rem ->
        ...
        { rem with
          lb_from = Some "eval_rec_bindings.bind_strict";
          lb_expr = Llet(Strict, id, rhs, rem) }
    | (id, Some(loc, shape), rhs) :: rem ->
        bind_strict rem
    ...
    | (id, Some(loc, shape), rhs) :: rem ->
        ...
        { rem with
          ...
          lb_expr =
            Lsequence(
              mk_lambda ?ty:None ?env:rem.lb_env
                ~from:"eval_rec_bindings.patch_forwards" @@
              Lapply(mod_prim "update_mod",
                     [shape;
                      mk_lambda ?ty:None ?env:rem.lb_env
                        ~from:"eval_rec_bindings.patch_forwards" @@ Lvar id;
                      rhs],
                     Location.none),
              rem); }
  in
...
let rec transl_module cc rootpath mexp =
  let mk l =
    mk_lambda ~ty:(Mod mexp.mod_type) ~env:mexp.mod_env
      ~from:"transl_module" l in
  match mexp.mod_type with
    Mty_alias _ ->
      { (apply_coercion mexp.mod_env Alias cc lambda_unit) with
        lb_tt_type = Some (Mod mexp.mod_type);
        lb_from = Some "transl_module"; }
  | _ ->
  match mexp.mod_desc with
    Tmod_ident (path,_) ->
      apply_coercion mexp.mod_env Strict cc
        (transl_path ~loc:mexp.mod_loc mexp.mod_env path)
  | Tmod_structure str ->
      transl_struct [] cc rootpath str
  | Tmod_functor( param, _, mty, body) ->
      let bodypath = functor_path rootpath param in
      let mty_t = match mty with None -> None | Some m -> Some m.mty_type in
      oo_wrap mexp.mod_env true
        ...
        | Tcoerce_functor(ccarg, ccres) ->
            let param' = Ident.create "funarg" in
            let body' = transl_module ccres bodypath body in
            mk @@
            Lfunction(Curried, [param'],
                      { body' with
                        lb_expr =
                          Llet(Alias, param,
                               apply_coercion mexp.mod_env Alias ccarg
                                 (mk_lambda
                                    ~ty:(Mod (Btype.default_mty mty_t))
                                    ~from:"transl_module"
                                    ~env:mexp.mod_env
                                  @@ Lvar param'),
                               body')});
  ...
  | Tmod_unpack(arg, _) ->
      apply_coercion mexp.mod_env Strict cc (Translcore.transl_exp arg)

and transl_struct fields cc rootpath str =
  transl_structure Env.empty (Mty_signature str.str_type) fields cc rootpath str.str_items

and transl_structure env mty fields cc rootpath items =
  let mk l = mk_lambda ~ty:(Mod mty) ~from:"transl_structure" l
  and mk_u l = mk_lambda ~from:"transl_structure" l in
  match items with
    [] ->
      begin match cc with
        Tcoerce_none ->
          mk @@ Lprim(Pmakeblock(0, Immutable),
                      List.map (fun id -> mk_u @@ Lvar id) (List.rev fields))
      | Tcoerce_structure(pos_cc_list, id_pos_list) ->
          ...
          let get_field pos = mk_u @@ Lvar v.(pos)
                ...
                List.map
                  (fun (pos, cc) ->
                    match cc with
                        Tcoerce_primitive p ->
                          transl_primitive Location.none None env p
                    | _ -> apply_coercion env Strict cc (get_field pos))
                  pos_cc_list))
          and id_pos_list =
  ...
  | item :: rem ->
      let env = item.str_env in
      match item.str_desc with
      | Tstr_eval (expr, _) ->
          mk_u @@
          Lsequence(transl_exp expr, transl_structure env mty fields cc rootpath rem)
      | Tstr_value(rec_flag, pat_expr_list) ->
          let ext_fields = rev_let_bound_idents pat_expr_list @ fields in
          transl_let rec_flag pat_expr_list
            (transl_structure env mty ext_fields cc rootpath rem)
      | Tstr_primitive descr ->
          record_primitive descr.val_val;
          transl_structure env mty fields cc rootpath rem
      | Tstr_type decls ->
          transl_structure env mty fields cc rootpath rem
      | Tstr_typext(tyext) ->
          let ids = List.map (fun ext -> ext.ext_id) tyext.tyext_constructors in
          transl_type_extension item.str_env rootpath tyext
            (transl_structure env mty (List.rev_append ids fields) cc rootpath rem)
      | Tstr_exception ext ->
          let id = ext.ext_id in
          let path = field_path rootpath id in
          mk_u @@
          Llet(Strict, id, transl_extension_constructor item.str_env path ext,
               transl_structure env mty (id :: fields) cc rootpath rem)
      | Tstr_module mb ->
          let id = mb.mb_id in
          mk_u @@
      ...
      | Tstr_include incl ->
          ...
          let rec rebind_idents pos newfields = function
              [] ->
                transl_structure env mty newfields cc rootpath rem
            | id :: ids ->
                mk_u @@
...
let transl_implementation module_name (str, cc) =
  ...
  mk_lambda
    ...
    ~from:"transl_implementation"
    ~env:Env.empty @@ (* Might be an environment containing all the imported compunits *)
...
let nat_toplevel_name id =
  try match Ident.find_same id !transl_store_subst with
    | {lb_expr =
         Lprim(Pfield pos,
               [{ lb_expr = Lprim(Pgetglobal glob, [])}]) } -> (glob,pos)
    | _ -> raise Not_found
  with Not_found ->
    fatal_error("Translmod.nat_toplevel_name: " ^ Ident.unique_name id)

let transl_store_structure glob map prims str =
  let mk_u = mk_lambda ?ty:None ~from:"transl_store_structure" in
  let last_env = ref Env.empty in
  let rec transl_store env rootpath subst = function
    [] ->
    ...
    | item :: rem ->
        let env = item.str_env in
        let mk_u = mk_u ~env in
        match item.str_desc with
  | Tstr_eval (expr, _attrs) ->
      mk_u @@
      Lsequence(subst_lambda subst (transl_exp expr),
                transl_store env rootpath subst rem)
  | Tstr_value(rec_flag, pat_expr_list) ->
      let ids = let_bound_idents pat_expr_list in
      let lam = transl_let rec_flag pat_expr_list (store_idents env ids) in
      mk_u @@
      Lsequence(subst_lambda subst lam,
                transl_store env rootpath (add_idents env false ids subst) rem)
  | Tstr_primitive descr ->
      record_primitive descr.val_val;
      transl_store env rootpath subst rem
  | Tstr_type decls ->
  ...
  | Tstr_typext(tyext) ->
      let ids = List.map (fun ext -> ext.ext_id) tyext.tyext_constructors in
      let lam =
        transl_type_extension item.str_env rootpath tyext (store_idents env ids)
      in
      mk_u @@
        Lsequence(subst_lambda subst lam,
                  transl_store env rootpath (add_idents env false ids subst) rem)
  | Tstr_exception ext ->
      ...
      mk_u @@
      Lsequence(mk_u @@
                Llet(Strict, id, subst_lambda subst lam, store_ident env id),
                transl_store env rootpath (add_ident env false id subst) rem)
  | Tstr_module{mb_id=id; mb_expr={mod_desc = Tmod_structure str}} ->
    let lam = transl_store env (field_path rootpath id) subst str.str_items in
    ...
    Lsequence(lam,
              ...
              Llet(Strict, id,
                   subst_lambda subst
                     (mk_u @@
                      Lprim(Pmakeblock(0, Immutable),
                            List.map (fun id -> mk_u @@ Lvar id)
                              (defined_idents str.str_items))),
                   mk_u @@ Lsequence(store_ident env id,
                                     transl_store env rootpath (add_ident env true id subst)
                                       rem)))
  | Tstr_module{mb_id=id; mb_expr=modl} ->
      ...
      Llet(Strict, id, subst_lambda subst lam,
           mk_u @@
           Lsequence(store_ident env id,
                     transl_store env rootpath (add_ident env true id subst) rem))
  | Tstr_recmodule bindings ->
      ...
      compile_recmodule
        ...
        bindings
        (mk_u @@
         Lsequence(store_idents env ids,
                   transl_store env rootpath (add_idents env true ids subst) rem))
  | Tstr_class cl_list ->
      let ids = List.map (fun (ci, _, _) -> ci.ci_id_class) cl_list in
      let lam =
        mk_u @@
  ...
  | Tstr_include incl ->
      ...
      let rec store_idents pos = function
        ...
        | id :: idl ->
            ...
            Llet(Alias, id, mk_u @@ Lprim(Pfield pos, [mk_u @@ Lvar mid]),
                 mk_u @@
                 Lsequence(store_ident env id, store_idents (pos + 1) idl)) in
      mk_u @@
      Llet(Strict, mid,
  ...
  and store_ident env id =
    try
      let (pos, cc) = Ident.find_same id map in
      let init_val = apply_coercion Env.empty Alias cc
          (mk_u ~env @@ Lvar id) in
      mk_u ~env @@
      Lprim(Psetfield(pos, false),
            [mk_u ~env @@ Lprim(Pgetglobal glob, []); init_val])
    with Not_found ->
      fatal_error("Translmod.store_ident: " ^ Ident.unique_name id)

  and store_idents env idlist =
    make_sequence (store_ident env) idlist

  and add_ident env may_coerce id subst =
    try
      let (pos, cc) = Ident.find_same id map in
      match cc with
        Tcoerce_none ->
          Ident.add id
            (mk_u ~env @@
             Lprim(Pfield pos,
                   [mk_u ~env @@ Lprim(Pgetglobal glob, [])])) subst
      | _ ->
          if may_coerce then subst else assert false
    with Not_found ->
      assert false

  and add_idents env may_coerce idlist subst =
    List.fold_right (add_ident env may_coerce) idlist subst

  and store_primitive env (pos, prim) cont =
    mk_u ~env @@
    Lsequence(mk_u ~env @@
              Lprim(Psetfield(pos, false),
                    [mk_u ~env @@
                     Lprim(Pgetglobal glob, []);
                     transl_primitive Location.none None env prim]),
              cont)

  in List.fold_right (store_primitive !last_env) prims
    (transl_store Env.empty (global_path glob) !transl_store_subst str.str_items)
...
let toploop_getvalue id =
  let mk_u l =
    mk_lambda ~from:"toploop_getvalue" l in
  mk_u @@
  Lapply(mk_u @@
         Lprim(Pfield toploop_getvalue_pos,
               [mk_u @@ Lprim(Pgetglobal toploop_ident, [])]),
         [mk_u @@ Lconst(Const_base(Const_string (toplevel_name id, None)))],
         Location.none)

let toploop_setvalue id lam =
  let mk_u l = mk_lambda ?ty:None ~from:"toploop_setvalue" ?env:None l in
  as_unit ~from:"toploop_setvalue" ?env:None @@
  Lapply(mk_u @@
...
let toploop_setvalue_id id =
  toploop_setvalue id
    (mk_lambda ?ty:None ~from:"toploop_setvalue_id" ?env:None @@ Lvar id)
...
let transl_toplevel_item item =
  let mk ?ty l =
    mk_lambda ?ty ~from:"transl_toplevel_item" ~env:item.str_env l in
  match item.str_desc with
    Tstr_eval (expr, _attrs) ->
      transl_exp expr
  | Tstr_value(rec_flag, pat_expr_list) ->
  ...
  | Tstr_include incl ->
      ...
      | id :: ids ->
          ...
          Lsequence(toploop_setvalue id
                      (mk ?ty:None @@ Lprim(Pfield pos, [mk ?ty:None @@ Lvar mid])),
...
let get_component = function
    None -> mk_lambda ?ty:None ~from:"get_component" ?env:None @@ Lconst const_unit
  | Some id -> mk_lambda ?ty:None ~from:"get_component" ?env:None @@ Lprim(Pgetglobal id, [])

let transl_package component_names target_name coercion =
  let mk_u l = mk_lambda ?ty:None ~from:"transl_package" ?env:None l in
  let components =
    mk_u @@
    Lprim(Pmakeblock(0, Immutable), List.map get_component component_names) in
  mk_u @@
  Lprim(Psetglobal target_name, [apply_coercion Env.empty Strict coercion components])
  (*
  let components =
    match coercion with
      Tcoerce_none ->
...
let transl_store_package component_names target_name coercion =
  let mk_u l = mk_lambda ~from:"transl_store_package" ?env:None l in
  let rec make_sequence fn pos arg =
    match arg with
      [] -> lambda_unit
    | hd :: tl -> mk_u @@ Lsequence(fn pos hd, make_sequence fn (pos + 1) tl) in
  match coercion with
    Tcoerce_none ->
  ...
  | Tcoerce_structure (pos_cc_list, id_pos_list) ->
      ...
      (List.length pos_cc_list,
       mk_u @@
       Llet (Strict, blk, apply_coercion Env.empty Strict coercion components,
             make_sequence
               (fun pos id ->
                  mk_u @@
                  Lprim(Psetfield(pos, false),
