...
let make_exit i = mk_lambda @@ Lstaticraise (i,[])

(* Introduce a catch, if worth it *)
let make_catch d k = match d.lb_desc with
| Lstaticraise (_,[]) -> k d
| _ ->
...
*)
    i,
    (fun body -> match body.lb_desc with
    | Lstaticraise (j,_) ->
        if i=j then handler else body
    | _ -> as_arg handler @@ Lstaticcatch (body,(i,[]),handler))
...
| []  -> assert false
| _::(({ lb_desc = Lvar v } as av,_) as arg)::rargs ->
...
let make_field_args loc binding_kind arg subpats first_pos last_pos argl =
  let rec make_args args pos =
    if pos > last_pos
    then argl
    else
      let prop, args = match args with
        | p :: rem -> (Env.summary p.pat_env, Val p.pat_type), rem
        | [] -> assert false
      in
      (mk_lambda ~prop @@
       Lprim(Pfield pos, [arg], loc), binding_kind)
      :: make_args args (pos + 1)
  in make_args subpats first_pos
      ...
      let newargs =
        if cstr.cstr_inlined <> None then
          ...
          Cstr_constant _ | Cstr_block _ ->
              make_field_args
                p.pat_loc Alias arg cstr_args 0 (cstr.cstr_arity - 1) argl
        | Cstr_unboxed -> (arg, Alias) :: argl
        | Cstr_extension _ ->
            make_field_args
              p.pat_loc Alias arg cstr_args 1 cstr.cstr_arity argl in
      {pm=
        {cases = []; args = newargs;
...
let make_variant_matching_nonconst p arg_prop lab def ctx = function
  ...
  | ((arg, _mut) :: argl) ->
      ...
      {pm=
         {cases = [];
          args =
            (mk_lambda ~prop:arg_prop @@
             Lprim(Pfield 1, [arg], p.pat_loc), Alias) :: argl;
          default=def} ;
        ctx=ctx ;
        pat = normalize_pat p}
  ...
  let rec divide = function
      ({pat_desc = Tpat_variant(lab, pato, _)} as p:: patl, action) :: rem ->
        ...
        then
          ...
          | Some pat ->
              let arg_prop = Env.summary pat.pat_env, Val pat.pat_type in
              add (make_variant_matching_nonconst p arg_prop lab def ctx)
                variants (=) (Cstr_block tag) (pat :: patl, action) al
        end
    | _ -> []
  in
  divide cl
...
let get_mod_field modname field =
  lazy (
    try
      ...
      let mty_prop =
        try Some (Env.summary env,
                  Module (Env.find_module (Path.Pident mod_ident) env).md_type)
        with Not_found -> None
      in
      let p, prop = try
        match Env.lookup_value (Longident.Lident field) env with
        | (Path.Pdot(_,_,i), ty) -> i, (Env.summary env, Val ty.val_type)
      ...
      mk_lambda ~prop @@
      Lprim(Pfield p,
            [{lb_desc = Lprim(Pgetglobal mod_ident, [], Location.none);
              lb_propagated = mty_prop;
             }],
            Location.none)
...
let inline_lazy_force_cond arg loc =
  ...
  let varg = as_arg arg @@ Lvar idarg in
  ...
  Llet(Strict, Pgenval, idarg, arg,
       ...
       Llet(Alias, Pgenval, tag,
            mk_lambda @@ Lprim(Pccall prim_obj_tag, [varg], loc),
            as_forced_arg @@
            Lifthenelse(
              ...
              Lprim(Pintcomp Ceq,
                    [mk_lambda @@ Lvar tag;
                     mk_lambda @@ Lconst(Const_base(Const_int Obj.forward_tag))],
                    loc),
              as_forced_arg @@
              ...
              Lifthenelse(
                ...
                Lprim(Pintcomp Ceq,
                      [mk_lambda @@ Lvar tag;
                       mk_lambda @@ Lconst(Const_base(Const_int Obj.lazy_tag))],
                      loc),
                as_forced_arg @@
         ...
         (Lswitch
            (varg,
             { sw_numconsts = 0; sw_consts = [];
               ...
               sw_blocks =
                 (* TODO: give a type to the tag? *)
                 [ (Obj.forward_tag, None,
                    as_forced_arg @@ Lprim(Pfield 0, [varg], loc));
                   (Obj.lazy_tag, None,
                    as_forced_arg @@
                    Lapply{ap_should_be_tailcall=false;
                           ap_loc=loc;
...
let make_tuple_matching loc p arity def = function
    [] -> fatal_error "Matching.make_tuple_matching"
  | (arg, _mut) :: argl ->
      let rec make_args tuple_args pos =
        if pos >= arity
        then argl
        else
          let tuple_arg_prop, tuple_args = match tuple_args with
            | p :: rem -> (Env.summary p.pat_env, Val p.pat_type), rem
            | [] -> assert false in
          (mk_lambda ~prop:tuple_arg_prop @@
           Lprim(Pfield pos, [arg], loc), Alias)
             :: make_args tuple_args (pos + 1) in
      {cases = []; args = make_args (get_args_tuple arity p []) 0 ;
...
let divide_tuple arity p ctx pm =
  divide_line
    (filter_ctx p)
    (make_tuple_matching p.pat_loc p arity)
...
let make_record_matching loc env all_labels def = function
  ...
  | ((arg, _mut) :: argl) ->
      let rec make_args pos =
        if pos >= Array.length all_labels then argl else begin
          ...
          let as_field =
            mk_lambda ~prop:(Env.summary env, Val lbl.lbl_arg) in
          let access =
            match lbl.lbl_repres with
            | Record_regular | Record_inlined _ ->
              as_field @@ Lprim (Pfield lbl.lbl_pos, [arg], loc)
            | Record_unboxed _ -> arg
            | Record_float ->
              as_field @@ Lprim (Pfloatfield lbl.lbl_pos, [arg], loc)
            | Record_extension ->
              as_field @@ Lprim (Pfield (lbl.lbl_pos + 1), [arg], loc)
          in
          let str =
...
let divide_record all_labels p ctx pm =
  ...
  divide_line
    (filter_ctx p)
    (make_record_matching p.pat_loc p.pat_env all_labels)
    get_args
    p ctx pm
...
let bind_sw arg k = match arg.lb_desc with
| Lvar _ -> k arg
| _ ->
    let id = Ident.create "switch" in
    as_arg arg @@ Llet (Strict,Pgenval,id,arg,k (as_arg arg @@ Lvar id))
           ...
           Lifthenelse
             (mk_lambda @@ Lprim
                (prim_string_notequal,
                 [arg; as_string @@ Lconst (Const_immstring s)], loc),
              k,lam))
        sw d)
...
let zero_lam  = mk_lambda @@ Lconst (Const_base (Const_int 0))

let tree_way_test loc arg lt eq gt =
  as_arg lt @@ Lifthenelse
    (mk_lambda @@ Lprim (Pintcomp Clt,[arg;zero_lam], loc),lt,
     as_arg lt @@ Lifthenelse(
       mk_lambda @@ Lprim (Pintcomp Clt,[zero_lam;arg], loc),gt,eq))
...
let rec do_make_string_test_tree loc arg sw delta d =
  ...
  if len <= strings_test_threshold+delta then
    ...
    let lt,(s,_p,act),gt = split len sw in
    bind_sw
      (as_int @@
       Lprim
         (prim_string_compare,
...
(* Store all other actions *)
  let sw =
    List.map  (fun (cst,p,act) -> cst,p,store.Switch.act_store act) sw in
...
(* Reconstruct default and switch list *)
  ...
  let sw = List.map (fun (cst,p,j) -> cst,p,acts.(j)) sw in
  !hs,sw,d
...
let rec do_tests_fail loc fail tst arg = function
  ...
  | (c, _prop, act)::rem ->
      ...
      Lifthenelse
        (mk_lambda @@
         Lprim (tst, [arg ; mk_lambda @@ Lconst (Const_base c)], loc),
...
let rec do_tests_nofail loc tst arg = function
  ...
  | (c,_prop,act)::rem ->
      ...
      Lifthenelse
        (mk_lambda @@
         Lprim (tst, [arg ; mk_lambda @@
                      Lconst (Const_base c)], loc),
...
let make_test_sequence loc fail tst lt_tst arg const_lambda_list =
  ...
  and split_sequence const_lambda_list =
    ...
    let test_seq1, test_seq2 =
      make_test_sequence list1, make_test_sequence list2 in
    as_arg test_seq1 @@
    Lifthenelse(
      mk_lambda @@
      Lprim(lt_tst,
...
module SArg = struct
  ...
  let make_prim p args = mk_lambda @@ Lprim (p,args,Location.none)
  let make_offset arg n = match n with
  | 0 -> arg
  | _ -> mk_lambda @@ Lprim (Poffsetint n,[arg],Location.none)

  let bind arg body =
    let newvar,newarg = match arg.lb_desc with
    | Lvar v -> v,arg
    | _      ->
        let newvar = Ident.create "switcher" in
        newvar, as_arg arg @@ Lvar newvar in
    bind Alias newvar arg (body newarg)
  let make_const i = mk_lambda @@ Lconst (Const_base (Const_int i))
  let make_isout h arg = mk_lambda @@ Lprim (Pisout, [h ; arg],Location.none)
  let make_isin h arg =
    mk_lambda @@ Lprim (Pnot,[make_isout h arg],Location.none)
  let make_if cond ifso ifnot = as_arg ifso @@ Lifthenelse (cond, ifso, ifnot)
  let make_switch arg cases acts =
    let l = ref [] in
    for i = Array.length cases-1 downto 0 do
      (* TODO: improve propagation *)
      l := (i, None, acts.(cases.(i))) ::  !l
    done ;
    mk_lambda @@ 
    Lswitch(arg,
...
(* Attempt sharing on all actions *)
  ...
  let consts =
    List.map
      (fun (i,p,e) -> i,p,store.Switch.act_store e)
      sw.sw_consts
  and blocks =
    List.map
      (fun (i,p,e) -> i,p,store.Switch.act_store e)
  ...
  !hs,
  { sw with
    sw_consts = List.map (fun (i,p,j) -> i,p,acts.(j)) consts ;
    sw_blocks = List.map (fun (i,p,j) -> i,p,acts.(j)) blocks ;
    sw_failaction = fail; }
...
| Partial ->
    ...
    | (_,idef)::_ ->
        Some (mk_lambda @@ Lstaticraise (idef,[])),jumps_singleton idef ctx
    | [] ->
  ...
  | ([],_)|(_,[]) ->
      List.fold_left
        (fun  (klist,jumps) (pats,i)->
          let action = mk_lambda @@ Lstaticraise (i,[]) in
          let klist =
            List.fold_right
              (fun pat r ->
                 (get_key_constr pat,
                  Some (Env.summary pat.pat_env, Val pat.pat_type),
                  action)::r)
              pats klist
          and jumps =
...
let combine_constant loc arg cst partial ctx def
  ...
  let lambda1 =
    ...
    | Const_int _ ->
        let int_lambda_list =
          List.map (function Const_int n,p,l -> n,p,l | _ -> assert false)
            const_lambda_list in
        call_switcher fail arg min_int max_int int_lambda_list
    | Const_char _ ->
        let int_lambda_list =
          List.map (function Const_char c, p, l -> (Char.code c, p, l)
            | _ -> assert false)
            const_lambda_list in
...
(* Note as the bytecode compiler may resort to dichotomic search,
   ...
   no duplicates *)
        ...
        let sw =
          List.map
            (fun (c,p,act) -> match c with
            | Const_string (s,_) -> s,p,act
            | _ -> assert false)
            const_lambda_list in
        let hs,sw,fail = share_actions_tree sw fail in
        let sw = List.map (fun (x,_,z) -> x,z) sw in
        hs (mk_lambda @@ Lstringswitch (arg,sw,fail,loc))
    | Const_float _ ->
...
let split_cases tag_lambda_list =
  let rec split_rec = function
    ...
    | (cstr, prop, act) :: rem ->
        ...
        match cstr with
          Cstr_constant n -> ((n, prop, act) :: consts, nonconsts)
        | Cstr_block n    -> (consts, (n, prop, act) :: nonconsts)
        | Cstr_unboxed    -> (consts, (0, prop, act) :: nonconsts)
...
let split_extension_cases tag_lambda_list =
  let rec split_rec = function
    ...
    | (cstr, prop, act) :: rem ->
        let (consts, nonconsts) = split_rec rem in
        match cstr with
          Cstr_extension(path, true) -> ((path, prop, act) :: consts, nonconsts)
        | Cstr_extension(path, false) -> (consts, (path, prop, act) :: nonconsts)
        | _ -> assert false in
    ...
    let lambda1 =
      ...
      let nonconst_lambda =
        ...
        | _ ->
            ...
            as_arg tests @@
            Llet(Alias, Pgenval,tag,
                 mk_lambda @@ Lprim(Pfield 0, [arg], loc), tests)
      in
        List.fold_right
          (fun (path, _prop, act) rem ->
             as_arg act @@
      ...
      | _ ->
          ...
          | (1, 1, [0, _, act1], [0, _, act2]) ->
           (* Typically, match on lists, will avoid isint primitive in that
              case *)
              as_arg act1 @@ Lifthenelse(arg, act2, act1)
          ...
          | (n, _, _, _) ->
              ...
              | Some act ->
                  as_arg act @@
                  Lifthenelse
                    (mk_lambda @@ Lprim (Pisint, [arg], loc),
                     call_switcher
                       fail_opt arg
                       0 (n-1) consts,
                     act)
(* Emit a switch, as bytecode implements this sophisticated instruction *)
              | None ->
                  ...
                  hs (mk_lambda @@ Lswitch (arg,sw)) in
    lambda1, jumps_union local_jumps total1
  end
...
let call_switcher_variant_constr loc fail arg int_lambda_list =
  ...
  let body = 
    call_switcher
      fail (mk_lambda @@ Lvar v) min_int max_int int_lambda_list in
  as_arg body @@
  Llet(Alias, Pgenval, v, mk_lambda @@ Lprim(Pfield 0, [arg], loc), body)
...
let combine_variant loc row arg partial ctx def
  ...
  let test_int_or_block arg if_int if_block =
    as_arg if_int @@
    Lifthenelse(mk_lambda @@ Lprim (Pisint, [arg], loc), if_int, if_block) in
...
let combine_array loc arg kind partial ctx def
  ...
  let lambda1 =
    ...
    let switch =
      call_switcher
        fail (mk_lambda @@ Lvar newvar)
        0 max_int len_lambda_list in
    bind
      Alias
      newvar
      (as_int @@ Lprim(Parraylength kind, [arg], loc))
      switch in
  lambda1, jumps_union local_jumps total1
...
let rec event_branch repr lam =
  begin match lam.lb_desc, repr with
    (_, None) ->
      lam
  ...
  | (key, cell) :: rem ->
      ...
      | _  ->
          try
            ...
            let prop =
              Some (Env.summary cell.pat.pat_env, Val cell.pat.pat_type) in
            ((key, prop, lambda1)::c_rem), total, (cell.pat::new_pats)
          with
  ...
  let rec do_rec r total_r = function
    ...
    | (mat,i,vars,pm)::rem ->
        begin try
          ...
          | _ ->
              do_rec
                (mk_lambda @@ Lstaticcatch (r,(i,vars), handler_i))
                (jumps_union
        ...
        | Unused ->
            do_rec
              (mk_lambda @@ Lstaticcatch (r, (i,vars), lambda_unit)) total_r rem
        end in
...
let rec lower_bind v arg lam = match lam.lb_desc with
| Lifthenelse (cond, ifso, ifnot) ->
    ...
    | false, true, false ->
        as_arg ifnot @@ Lifthenelse (cond, lower_bind v arg ifso, ifnot)
    | false, false, true ->
        as_arg ifnot @@ Lifthenelse (cond, ifso, lower_bind v arg ifnot)
    | _,_,_ -> bind Alias v arg lam
    end
| Lswitch (ls,({sw_consts=[i,p,act] ; sw_blocks = []} as sw))
    when not (approx_present v ls) ->
    as_arg act @@
    Lswitch (ls, {sw with sw_consts = [i,p,lower_bind v arg act]})
| Lswitch (ls,({sw_consts=[] ; sw_blocks = [i,p,act]} as sw))
    when not (approx_present v ls) ->
    as_arg act @@
    Lswitch (ls, {sw with sw_blocks = [i,p,lower_bind v arg act]})
| Llet (Alias, k, vv, lv, l) ->
    if approx_present v lv then
      bind Alias v arg lam
    else
      as_arg l @@ Llet (Alias, k, vv, lv, lower_bind v arg l)
| _ ->
    bind Alias v arg lam

let bind_check str v arg lam = match str,arg.lb_desc with
| _, Lvar _ ->bind str v arg lam
| Alias,_ -> lower_bind v arg lam
| _,_     -> bind str v arg lam

let comp_exit ctx m = match m.default with
| (_,i)::_ -> mk_lambda @@ Lstaticraise (i,[]), jumps_singleton i ctx
                ...
                try
                  let li,total_i =
                    comp_fun
                      ...
                      ctx_i arg pm in
                  c_rec
                    (as_arg body @@ Lstaticcatch (body,(i,[]),li))
                    (jumps_union total_i total_rem)
                    rem
                with
                | Unused ->
                    c_rec (as_arg body @@ Lstaticcatch (body,(i,[]),lambda_unit))
                      total_rem  rem
            end in
   try
...
let arg_to_var arg cls = match arg.lb_desc with
| Lvar v -> v,arg
| _ ->
...
let check_total total lambda i handler_fun =
  if jumps_is_empty total then
    ...
    as_arg lambda @@ Lstaticcatch(lambda, (i,[]), handler_fun())
  end
...
let for_trywith param pat_act_list =
  compile_matching None
    (fun () -> mk_lambda @@ Lprim(Praise Raise_reraise, [param], Location.none))
...
let rec map_return f l =
  match l.lb_desc with
  | Llet (str, k, id, l1, l2) ->
      as_arg l @@ Llet (str, k, id, l1, map_return f l2)
  | Lletrec (l1, l2) ->
      as_arg l @@ Lletrec (l1, map_return f l2)
  | Lifthenelse (lcond, lthen, lelse) ->
      as_arg l @@ Lifthenelse (lcond, map_return f lthen, map_return f lelse)
  | Lsequence (l1, l2) ->
      as_arg l @@ Lsequence (l1, map_return f l2)
  | Levent (l, ev) -> as_arg l @@ Levent (map_return f l, ev)
  | Ltrywith (l1, id, l2) -> as_arg l @@ Ltrywith (map_return f l1, id, map_return f l2)
  | Lstaticcatch (l1, b, l2) ->
      as_arg l @@ Lstaticcatch (map_return f l1, b, map_return f l2)
  | Lstaticraise _ | Lprim(Praise _, _, _) -> l
  | _ -> f l
...
let assign_pat opt nraise catch_ids loc pat lam =
  let rec collect acc pat lam = match pat.pat_desc, lam.lb_desc with
  ...
  | Tpat_tuple patl, Lconst(Const_block(_, scl)) ->
      ...
      let collect_const acc pat sc =
        collect acc pat (mk_lambda @@ Lconst sc) in
  ...
  let exit =
    ...
    let fresh_var id = mk_lambda @@ Lvar (Ident.find_same id tbl) in
    mk_lambda @@ Lstaticraise(nraise, List.map fresh_var catch_ids)
  in
...
let for_tupled_function loc paraml pats_act_list partial =
  ...
  let pm =
    { cases = pats_act_list;
      args = List.map (fun id -> (mk_lambda @@ Lvar id, Strict)) paraml ;
...
(* Once for checking that compilation is possible *)
      ...
      let args =  List.map (fun id -> mk_lambda @@ Lvar id, Alias) idl in
...
let param_to_var param = match param.lb_desc with
| Lvar v -> v,None
...
let for_multiple_match loc paraml pat_act_list partial =
  ...
  let paraml = List.map (fun (v,_) -> mk_lambda @@ Lvar v) v_paraml in
