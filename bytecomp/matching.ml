...
let make_exit ?ty ?env i = mk_lambda ?ty ~from:"make_exit" ?env @@ Lstaticraise (i,[])
...
| _ ->
    ...
    as_arg d ~from:"make_catch" @@
    Lstaticcatch (k (make_exit ?ty:d.lb_tt_type e),(e,[]),d)
    ...
    let rec do_cases = function
      | ({pat_desc=Tpat_or _} as orp::patl, action)::rem ->
          ...
          let mk_new_action ?ty ty_vars vs =
            mk_lambda ~from:"precompile_or" ?ty ?env:None @@
            Lstaticraise
              (or_num, List.map2 (fun v ty ->
                   mk_lambda ?ty ~from:"precompile_or" ?env:None @@ Lvar v) vs ty_vars) in
...
let make_field_args binding_kind arg cstr_args first_pos last_pos argl =
  let rec make_args pos cstr_args =
    if pos > last_pos
    then argl
    else
      let arg_ty, arg_env, cstr_args =
        (List.hd cstr_args).pat_type, (List.hd cstr_args).pat_env, List.tl cstr_args in
      (mk_lambda ~ty:(Val arg_ty) ~from:"make_field_args" ~env:arg_env @@
          Lprim(Pfield pos, [arg]), binding_kind) :: make_args (pos + 1) cstr_args
  in make_args first_pos cstr_args
...
let make_constr_matching p def ctx = function
  ...
  | ((arg, mut) :: argl) ->
      let cstr = pat_as_constr p in
      let cstr_args = get_args_constr p [] in
      let newargs =
        match cstr.cstr_tag with
          Cstr_constant _ | Cstr_block _ ->
            make_field_args Alias arg cstr_args 0 (cstr.cstr_arity - 1) argl
        | Cstr_extension _ ->
            make_field_args Alias arg cstr_args 1 cstr.cstr_arity argl in
      {pm=
        {cases = []; args = newargs;
...
let make_variant_matching_nonconst p ty lab def ctx = function
  ...
  | ((arg, mut) :: argl) ->
      ...
      {pm=
         {cases = [];
          args = (mk_lambda ~ty ~from:"make_variant_matching_nonconst" ?env:None @@
                  Lprim(Pfield 1, [arg]), Alias) :: argl;
          default=def} ;
        ctx=ctx ;
        pat = normalize_pat p}
...
let get_arg_variant_type p = match p.pat_desc with
| Tpat_variant(_, Some pat, _) -> Val pat.pat_type
| Tpat_variant(_, None , _) -> assert false (* called only in case of nonconst variant *)
|  _ -> assert false
  ...
  let rec divide = function
      ({pat_desc = Tpat_variant(lab, pato, _)} as p:: patl, action) :: rem ->
        ...
        then
          ...
          | Some pat ->
              let ty = get_arg_variant_type p in
              add (make_variant_matching_nonconst p ty lab def ctx) variants
...
let get_mod_field modname field =
  lazy (
    try
      ...
      let mty =
        try Some (Mod (Env.find_module (Path.Pident mod_ident) env).md_type)
              with Not_found -> None
      in
      let p, ty = try
      ...
      in
      mk_lambda ~ty:(Val ty) ~from:"get_mod_field" ~env @@
      Lprim(Pfield p,
            [{ lb_expr = Lprim(Pgetglobal mod_ident, []);
               lb_tt_type = mty;
               lb_from = Some "get_mod_field";
               lb_env = Some env;
             }])
...
let inline_lazy_force_cond arg loc =
  let from = "inline_lazy_force_cond" in
  let idarg = Ident.create "lzarg" in
  let varg = as_arg ~from ?env:arg.lb_env arg @@ Lvar idarg in
  let as_forced_arg =
    as_constr_arg1 ~from ?env:varg.lb_env varg
      (fun p ty -> if p == Predef.path_lazy_t then ty else raise Not_found) in
  ...
  Llet(Strict, idarg, arg,
       as_forced_arg @@
       Llet(Alias, tag,
            as_arg ~from:"inline_lazy_force_cond+(of type int)" ?env:None arg @@
            Lprim(Pccall prim_obj_tag, [varg]),
            as_forced_arg @@
            Lifthenelse(
              (* if (tag == Obj.forward_tag) then varg.(0) else ... *)
              as_bool ~from ?env:arg.lb_env @@
              Lprim(Pintcomp Ceq,
                    [as_arg arg ~from @@ Lvar tag;
                     as_int ~from ?env:arg.lb_env @@
                     Lconst(Const_base(Const_int Obj.forward_tag))]),
              ...
              Lifthenelse(
                ...
                as_bool ~from ?env:arg.lb_env @@
                Lprim(Pintcomp Ceq,
                      [as_arg ~from ?env:arg.lb_env arg @@ Lvar tag;
                       as_int ~from ?env:arg.lb_env @@
                       Lconst(Const_base(Const_int Obj.lazy_tag))]),
...
let inline_lazy_force_switch arg loc =
  let from = "inline_lazy_force_switch" in
  let as_forced_arg =
    as_constr_arg1 ~from ?env:arg.lb_env arg
      (fun p ty -> if p == Predef.path_lazy_t then ty else raise Not_found) in
  let idarg = Ident.create "lzarg" in
  let varg = as_arg ~from ?env:arg.lb_env arg @@ Lvar idarg in
  let force_fun = Lazy.force code_force_lazy_block in
          ...
          Lswitch
            (varg,
             { sw_numconsts = 0; sw_consts = [];
               ...
               sw_blocks =
                 [ (Obj.forward_tag,
                    mk_switch_extr_as arg ~from:"inline_lazy_force_switch",
                    as_forced_arg @@ Lprim(Pfield 0, [varg]));
                   (Obj.lazy_tag,
                    mk_switch_extr_as arg ~from:"inline_lazy_force_switch",
                    as_forced_arg @@ Lapply(force_fun, [varg], loc)) ];
...
let make_tuple_matching arity p def = function
    [] -> fatal_error "Matching.make_tuple_matching"
  | (arg, mut) :: argl ->
      let rec make_args pos tpl_args =
        if pos >= arity
        then argl
        else
          let arg_ty, tpl_args =
            (List.hd tpl_args).pat_type, List.tl tpl_args in
          (as_tuple_arg ~from:"make_tuple_matching" arg pos @@
              Lprim(Pfield pos, [arg]), Alias) :: make_args (pos + 1) tpl_args in
      {cases = []; args = make_args 0 (get_args_tuple arity p []);
  ...
  | ((arg, mut) :: argl) ->
      let rec make_args pos =
        if pos >= Array.length all_labels then argl else begin
          ...
          let as_field =
            mk_lambda ~ty:(Val lbl.lbl_arg) ~from:"make_record_matching" ?env:None in
          let access =
  ...
  | ((arg, mut) :: argl) ->
      ...
      let rec make_args pos =
        ...
        then argl
          ...
          (as_constr_arg1 ~from:"make_array_matching" ?env:None arg (fun p ty ->
               if p == Predef.path_array then ty else raise Not_found) @@
           Lprim(Parrayrefu kind,
                 [arg; as_int ~from:"make_array_matching" ?env:None @@
...
let bind_sw arg k = match arg.lb_expr with
| Lvar _ -> k arg
| _ ->
    let id = Ident.create "switch" in
    as_arg arg @@
    Llet (Strict,id,arg,k (as_arg arg @@ Lvar id))
...
let make_string_test_sequence arg sw d =
  ...
  bind_sw arg
    (fun arg ->
      List.fold_right
        (fun (s,_,lam) k ->
           as_arg ~from:"make_string_test" ?env:None k @@
          Lifthenelse
            (as_bool ~from:"make_string_test" ?env:None @@
             Lprim
               (prim_string_notequal,
                [arg; as_string ~from:"make_string_test" ?env:None @@
...
let zero_lam  = as_int ~from:"zero_lam" ?env:None @@ Lconst (Const_base (Const_int 0))

let tree_way_test arg lt eq gt =
  as_arg ~from:"tree_way_test" ?env:None lt @@
  Lifthenelse
    (as_bool ~from:"tree_way_test" ?env:None  @@
     Lprim (Pintcomp Clt,[arg;zero_lam]),lt,
     as_arg ~from:"tree_way_test" ?env:None lt @@
     Lifthenelse(as_bool ~from:"tree_way_test" ?env:None @@
                 Lprim (Pintcomp Clt,[zero_lam;arg]),gt,eq))

(* Dichotomic tree *)


let rec do_make_string_test_tree arg (sw : ('a * 'b * 'c) list) delta d =
  let len = List.length sw in
  if len <= strings_test_threshold+delta then
    make_string_test_sequence arg sw d
  else
    let lt,(s,extr,act),gt = split len sw in
    bind_sw
      (as_int ~from:"do_make_string_test" ?env:None @@
       Lprim
         (prim_string_compare,
          [arg; as_string ~from:"do_make_string_test" ?env:None @@
           Lconst (Const_immstring s)];))
      (fun r ->
        tree_way_test r
...
(* Entry point *)
let expand_stringswitch arg (sw: ('a * 'b * 'c) list) d = match d with
| None ->
    bind_sw arg
...
(* Store all other actions *)
  let sw =
    List.map  (fun (cst,e,act) -> cst,e,store.Switch.act_store act) sw in
...
let rec do_tests_fail fail tst arg = function
  | [] -> fail
  | (c, extr, act)::rem ->
      as_arg ~from:"do_tests_fail" ?env:None act @@
      Lifthenelse
        (as_bool ~from:"do_tests_fail" ?env:None @@
         Lprim (tst, [arg ;
                      mk_lambda_as_extr extr ~from:"do_test_fail" ?env:None @@
                      Lconst (Const_base c)]),
...
let rec do_tests_nofail tst arg = function
  ...
  | [_, _,act] -> act
  | (c, extr, act)::rem ->
      as_arg ~from:"do_tests_nofail" ?env:None act @@
      Lifthenelse
        (as_bool ~from:"do_test_nofail" ?env:None @@
         Lprim (tst, [arg ;
                      mk_lambda_as_extr extr ~from:"do_test_fail" ?env:None @@
                      Lconst (Const_base c)]),
...
let make_test_sequence fail tst lt_tst arg (const_lambda_list : ('a * 'b *'c) list) =
  ...
  and split_sequence const_lambda_list =
    ...
    let seq1, seq2 = make_test_sequence list1, make_test_sequence list2 in
    as_arg ~from:"make_test_sequence" ?env:None seq1 @@
    Lifthenelse(
      as_bool ~from:"make_test_sequence" ?env:None @@
      Lprim(lt_tst,[arg;
                    as_arg ~from:"make_test_sequence" ?env:None arg @@
                    Lconst(Const_base (Misc.fst3(List.hd list2)))]),
      seq1, seq2)
  in
...
module SArg = struct
  ...
  let make_prim p args = mk_lambda ?ty:None ~from:"make_prim" ?env:None @@ Lprim (p,args)
  let make_offset arg n = match n with
  | 0 -> arg
  | _ -> as_arg ~from:"SArg.make_offset" arg @@ Lprim (Poffsetint n,[arg])

  let bind arg body =
    let newvar,newarg = match arg.lb_expr with
    | Lvar v -> v,arg
    | _      ->
        let newvar = Ident.create "switcher" in
        newvar,as_arg ~from:"SArg.bind_arg" arg @@ Lvar newvar in
    bind Alias newvar arg (body newarg)
  let make_const i =
    as_int ~from:"SArg.make_const" ?env:None @@ Lconst (Const_base (Const_int i))
  let make_isout h arg =
    as_bool ~from:"SArg.make_isout" ?env:None @@ Lprim (Pisout, [h ; arg])
  let make_isin h arg =
    as_bool ~from:"SArg.make_isin" ?env:None @@ Lprim (Pnot,[make_isout h arg])
  let make_if cond ifso ifnot =
    as_arg ~from:"SArg.make_if" ?env:None ifso @@ Lifthenelse (cond, ifso, ifnot)
  let make_switch arg cases acts =
    let l = ref [] in
    for i = Array.length cases-1 downto 0 do
      (* TODO: a better propagation *)
      l := (i, mk_switch_extr ~from:"SArg.make_switch" (), acts.(cases.(i))) ::  !l
    done ;
    as_arg ~from:"SArg.make_switch" acts.(0) @@
    Lswitch(arg,
...
(* Attempt sharing on all actions *)
  ...
  let consts =
    List.map
      (fun (i, extr, e) -> i, extr, store.Switch.act_store e)
      sw.sw_consts
  and blocks =
    List.map
      (fun (i, extr, e) -> i, extr, store.Switch.act_store e)
  ...
  !hs,
  { sw with
    sw_consts = List.map (fun (i,j,k) -> i,j,acts.(k)) consts ;
    sw_blocks = List.map (fun (i,j,k) -> i,j,acts.(k)) blocks ;
    sw_failaction = fail; }
...
| None ->
    ...
    let seen (_,_,l) = match as_simple_exit l with
    | Some i ->
    ...
    if !max >= 3 then
      ...
      let remove =
        List.filter
          (fun (_,_,lam) -> match as_simple_exit lam with
          | Some j -> j <> default
          | None -> true) in
      {sw with
...
module Switcher = Switch.Make(
  struct
    include SArg
    let make_exit = make_exit ?ty:None ?env:None
  end)
open Switch

let lambda_of_int i =
  as_int ~from:"lambda_of_int" ?env:None @@ Lconst (Const_base (Const_int i))
...
let mk_res get_key env last_choice idef cant_fail ctx =
  ...
  | [p] when group_var p ->
      env,
      Some (mk_lambda ?ty:None ~from:"mk_res" ?env:None 
            @@ Lstaticraise (idef,[])),
  ...
  let klist,jumps =
    List.fold_right
      (fun (i,cant_fail,pats) (klist,jumps) ->
         let act = mk_lambda ?ty:None ~from:"mk_res" ?env:None @@
           Lstaticraise (i,[])
...
| Partial ->
    ...
    | (_,idef)::_ ->
        Some (mk_lambda ?ty ~from:"mk_failaction" ?env:None @@
...
and mk_failaction_pos partial seen ctx defs : ('a * 'b * 'c) list * 'd=
  if dbg then begin
    prerr_endline "**POS**" ;
    pretty_def defs ;
    ()
  end ;
  let rec scan_def env to_test defs : ('a * 'b * 'c) list * 'd = match to_test,defs with
  | ([],_)|(_,[]) ->
      List.fold_left
        (fun  (klist,jumps) (pats,i)->
           let action ty =
             mk_lambda ?ty ~from:"mk_failaction" ?env:None @@ Lstaticraise (i,[]) in
          let klist =
            List.fold_right
              (fun pat r ->
                 (get_key_constr pat,
                  mk_switch_extr ~ty:pat.pat_type ~from:"mk_fail_action" (),
                  action (Some (Val pat.pat_type)))::r)
              pats klist
          and jumps =
...
let combine_constant arg cst partial ctx def
  ...
  let lambda1 =
    ...
    | Const_int _ ->
        let int_lambda_list =
          List.map (function Const_int n, e, l -> n, e, l | _ -> assert false)
            const_lambda_list in
        call_switcher fail arg min_int max_int int_lambda_list
    | Const_char _ ->
        let int_lambda_list =
          List.map (function Const_char c, e, l -> (Char.code c, e, l)
            | _ -> assert false)
            const_lambda_list in
...
(* Note as the bytecode compiler may resort to dichotmic search,
   ...
   no duplicates *)
        ...
        let sw =
          List.map
            (fun (c,extr,act) -> match c with
            | Const_string (s,_) -> s,extr,act
            | _ -> assert false)
            const_lambda_list in
        let hs,sw,fail = share_actions_tree sw fail in
        let ty = match sw with [] -> None | (_, _, l) :: _ -> l.lb_tt_type in
        hs (mk_lambda ?ty ~from:"combine_constant" ?env:None
  ...
  let rec split_rec = function
    ...
    | (cstr, extr, act) :: rem ->
        ...
        match cstr with
          Cstr_constant n ->
            ((n,
              mk_switch_extr ?ty:extr.pattern_type ~from:"split_cases" (),
...
let split_extension_cases tag_lambda_list =
  let rec split_rec = function
    ...
    | (cstr, extr, act) :: rem ->
        let (consts, nonconsts) = split_rec rem in
        match cstr with
          Cstr_extension(path, true) -> ((path, extr, act) :: consts, nonconsts)
        | Cstr_extension(path, false) -> (consts, (path, extr, act) :: nonconsts)
        | _ -> assert false in
  split_rec tag_lambda_list


let combine_constructor arg ex_pat cstr partial ctx def
    ((tag_lambda_list : ('a * 'b * 'c) list), total1, pats) =
    ...
    let lambda1 =
      ...
      let default, consts, nonconsts =
        ...
        | None ->
            ...
            | _, (_, extr, act)::rem -> act, consts, rem
            | (_, extr, act)::rem, _ -> act, rem, nonconsts
            | _ -> assert false
            end
      ...
      let nonconst_lambda =
        ...
        | _ ->
            ...
            let tests =
              List.fold_right
                (fun (path, extr, act) rem ->
                   let tag =
                     mk_lambda_as_extr extr ~from:"combine_constructor" ?env:None
                     @@ Lvar tag in
                   as_arg ~from:"combine_constructor" act ?env:None @@
                   Lifthenelse(
                     as_bool ~from:"combine_constructor" ?env:None @@
                     Lprim(Pintcomp Ceq,
                           [tag;
                            transl_path ex_pat.pat_env ?ty:tag.lb_tt_type path]),
                     act, rem))
                nonconsts
                default
            in
            as_arg ~from:"combine_constructor" tests ?env:None @@
            Llet(Alias, tag, as_int ~from:"combine_constructor(get tag)" ?env:None @@
                 Lprim(Pfield 0, [arg]), tests)
      in
        List.fold_right
          (fun (path, extr, act) rem ->
             let ty = match extr.pattern_type with
                 None -> None
               | Some ty -> Some (Val ty) in
             as_arg ~from:"combine_constructor" ?env:None act @@
             Lifthenelse(
               as_bool ~from:"combine_constructor" ?env:None @@
               Lprim(Pintcomp Ceq,
                     [arg; transl_path ex_pat.pat_env ?ty path]),
               act, rem))
          consts
          nonconst_lambda
    in
    ...
    let lambda1 =
      ...
      | _ ->
          ...
          | (1, 1, [0, _, act1], [0, _, act2]) ->
              as_arg ~from:"combine_constructor" ?env:None act1 @@
...
(* Emit a switch, as bytecode implements this sophisticated instruction *)
                  ...
                  hs (mk_lambda ?ty:None ~from:"combine_constructor" ?env:None @@
                      Lswitch (arg,sw))
              | Some act ->
                  as_arg ~from:"combine_constructor" ?env:None act @@
                  Lifthenelse
...
let call_switcher_variant_constr fail arg int_lambda_list =
  ...
  let body = 
    call_switcher
      fail (as_int ~from:"call_switcher_variant_constr (variant tag)" ?env:None
            @@ Lvar v)
      min_int max_int int_lambda_list in
  as_arg ~from:"call_switcher_variant_constr" ?env:None body @@
  Llet(Alias, v, as_int ~from:"call_switcher_variant_constr (variant_tag)" ?env:None @@
       Lprim(Pfield 0, [arg]), body)
...
let combine_variant row arg partial ctx def (tag_lambda_list, total1, pats) =
  ...
  let test_int_or_block arg if_int if_block =
    as_arg ~from:"combine_variant.int_or_block" ?env:None if_int @@
    Lifthenelse(as_bool ~from:"combine_variant" ?env:None @@ Lprim (Pisint, [arg]),
                if_int, if_block) in
  ...
  | _,_ ->
      ...
      | ([n, _, act1], [m, _, act2]) when fail=None ->
          (* /!\ Here, the type is lost due to the optimization that does not
             test the tag *)
...
let combine_array arg kind partial ctx def
  ...
  let lambda1 =
    ...
    let switch =
      call_switcher
        fail (as_int ~from:"combine_array" ?env:None @@ Lvar newvar)
        0 max_int len_lambda_list in
    bind
      Alias newvar (as_int ~from:"combine_array" ?env:None @@
                    Lprim(Parraylength kind, [arg])) switch in
  lambda1, jumps_union local_jumps total1

(* Insertion of debugging events *)

let rec event_branch repr lam =
  begin match lam.lb_expr, repr with
    (_, None) ->
      lam
  | (Levent(lam', ev), Some r) ->
      incr r;
      as_arg ~from:"event_branch" ?env:None lam @@
  ...
  | (key, cell) :: rem ->
      ...
      | _  ->
          try
            ...
            ((key,
              mk_switch_extr
                ~ty:cell.pat.pat_type
                ~from:"compile_list"
                ~env:cell.pat.pat_env (),
              lambda1)::c_rem),
            total, (cell.pat::new_pats)
          with
  ...
  let rec do_rec r total_r = function
    ...
    | (mat,i,vars,pm)::rem ->
        begin try
          ...
          | _ ->
              do_rec
                (as_arg ~from:"compile_orhandlers" ?env:None r @@
        ...
        with
        | Unused ->
            do_rec (as_arg ~from:"compile_orhandlers" ?env:None r @@
                    Lstaticcatch (r, (i,vars), lambda_unit)) total_r rem
        end in
...
let rec lower_bind v arg lam = match lam.lb_expr with
...
| Lswitch (ls,({sw_consts=[i, extr, act] ; sw_blocks = []} as sw))
    when not (approx_present v ls) ->
    as_arg ~from:"lower_bind" lam @@
    Lswitch (ls, {sw with sw_consts = [i, extr, lower_bind v arg act]})
| Lswitch (ls,({sw_consts=[] ; sw_blocks = [i, extr, act]} as sw))
    when not (approx_present v ls) ->
    as_arg ~from:"lower_bind" lam @@
    Lswitch (ls, {sw with sw_blocks = [i, extr, lower_bind v arg act]})
| Llet (Alias, vv, lv, l) ->
...
let bind_check str v arg lam = match str, arg.lb_expr with
...
let comp_exit ctx m = match m.default with
  | (_,i)::_ ->
      mk_lambda ?ty:None ~from:"comp_exit" ?env:None @@
                ...
                try
                  let li,total_i =
                    comp_fun
                      ...
                      ctx_i arg pm in
                  c_rec
                    (as_arg ~from:"comp_match_handlers" body @@
...
let arg_to_var arg cls = match arg.lb_expr with
| Lvar v -> v,arg
| _ ->
    let v = name_pattern "match" cls in
    v, as_arg ~from:"arg_to_var" arg @@ Lvar v
...
let check_total total lambda i handler_fun =
  if jumps_is_empty total then
    ...
    as_arg ~from:"check_total" lambda @@
...
let partial_function loc () =
  ...
  let mk l = mk_lambda ~from:"partial_function" ?env:None l in
  mk @@
  Lprim(Praise Raise_regular,
        [mk @@
         Lprim(Pmakeblock(0, Immutable),
               [transl_normal_path ?ty:(Some (Val Predef.type_exn))
                  Env.empty Predef.path_match_failure;
                mk @@ Lconst(Const_block(0,
...
let for_trywith param pat_act_list =
  compile_matching Location.none None
    (fun () -> mk_lambda ?ty:None ~from:"for_trywith" ?env:None @@
      Lprim(Praise Raise_reraise, [param]))
...
let for_tupled_function loc paraml pats_act_list partial =
  ...
  let pm =
    { cases = pats_act_list;
      args = List.map (fun id ->
          (mk_lambda ?ty:None ~from:"for_tupled_function" ?env:None @@
           Lvar id, Strict)) paraml ;
...
let do_for_multiple_match loc paraml pat_act_list partial =
  let repr = None in
  let partial = check_partial pat_act_list partial in
  let mk ?ty = mk_lambda ?ty ~from:"do_for_mutliple_match" ?env:None in
  let raise_num,pm1 =
    match partial with
    | Partial ->
        let raise_num = next_raise_count () in
        raise_num,
        { cases = List.map (fun (pat, act) -> ([pat], act)) pat_act_list;
          args = [mk ?ty:None @@ Lprim(Pmakeblock(0, Immutable), paraml), Strict] ;
...
(* Once for checking that compilation is possible *)
      ...
      let args =  List.map (fun id -> mk ?ty:None @@ Lvar id, Alias) idl in
...
| _ ->
    let v = name_pattern "match" cls in
    v, as_arg ~from:"arg_to_var" ?env:None arg @@ Lvar v


let param_to_var param = match param.lb_expr with
| Lvar v -> v,None
...
let for_multiple_match loc paraml pat_act_list partial =
  ...
  let paraml = List.map (fun (v,_) ->
      mk_lambda ?ty:None ~from:"for_multiple_match" ?env:None @@ Lvar v) v_paraml in
