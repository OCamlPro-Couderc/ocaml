...
let check_recursive_lambda idlist lam =
  let rec check_top idlist l =
    match l.lb_expr with
    | Lvar v -> not (List.mem v idlist)
    | Llet (_, _, _, _) when check_recursive_recordwith idlist l ->
        true
  ...
  and check_recursive_recordwith idlist (l : Lambda.lambda) =
    match l.lb_expr with
    | Llet (Strict, id1,
            { lb_expr = Lprim (Pduprecord _, [e1])}, body) ->
       check_top idlist e1
       && check_recordwith_updates idlist id1 body
    | _ -> false

  and check_recordwith_updates idlist id1 l =
    match l.lb_expr with
    | Lsequence ({lb_expr =
                    Lprim ((Psetfield _ | Psetfloatfield _),
                           [{ lb_expr = Lvar id2 }; e1])}, cont)
...
let extract_constant = function
    { lb_expr = Lconst sc; lb_tt_type = ty }-> sc, ty
...
let event_before exp lam = match lam.lb_expr with
...
and transl_exp0 e =
  ...
  | Texp_array expr_list ->
      ...
      begin try
        ...
        let cl, tys = List.map extract_constant ll |> List.split in
        let master =
          match kind with
...
and transl_guard guard rhs =
  ...
  | Some cond ->
      event_before cond @@
      { expr with lb_expr = Lifthenelse(transl_exp cond, expr, staticfail) }
