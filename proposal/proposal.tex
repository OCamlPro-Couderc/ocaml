...
\texttt{array.ml} when computing dependencies. Module aliases allows
transitivity of modules, in other word when a module uses an alias of another,
it is dependent of the original module. It can be easily proved, using
\texttt{ocamlobjinfo} on \texttt{std\_array.cmo}, which uses Std.List:
...
Std\_array. Those dependencies are ``transparent''.
...
goal of this impplementation). However, those bits of namespaces are only on the
...
can be hard to write according to this namespace strategy. 
