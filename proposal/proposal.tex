...
ns_conf ::=
    | "(" modules_constraints ")" "of" mod_long_ident 
                                                      modules constraints
    | "(" modules_constraints ")" "of" mod_long_ident instance_args 
...
add the module (it is only useful when using the wildcard), and aliasing allows
to have module from multiple namespace with the same original name.
...
in namespace Myserver
  with (Lwt) of Ocsigen
  and (Protocol) of MyLib.Http(Async_monad = Lwt)
...
\end{OCaml}
...
\begin{OCaml}
(** bench.ml *)
with (Parser, Solver -> Solve_Dpll) of MySolver(Sat = Dpll)
and (Solver -> Solve_Cdcl) of MySolver(Sat = Cdcl)
