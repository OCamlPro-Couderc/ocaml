...
let init_env native module_name =
...
let raw_with_info ~gen_env ~native ~tool_name ~source_file ~output_prefix ~dump_ext k =
  ...
  let env =
    if gen_env then Some (init_env native module_name)
    else None in
...
let typecheck_intf info ast =
  ...
  let tsg =
    ast
    |> Typemod.type_interface info.source_file env
...
let typecheck_rec_intf infos asts =
  ...
  let ppf_dump, native = match infos with
      [] -> assert false
    | i :: _ -> i.ppf_dump, i.native
  in
  let env = init_env native "*recmod*" in
  let tsgs, recenv =
    asts
    |> List.map2 (fun info ast ->
        info.module_name, ast, Location.in_file info.source_file) infos
